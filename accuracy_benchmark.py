...
def load_dataset(path=None, limit=None):
    ...
    if not isinstance(dataset, list):
        raise ValueError(
            "Dataset must be a list of objects with 'prompt' and 'expected'."
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default=None, help="Optional JSON or JSONL dataset"
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="Random seed for reproducible runs"
    )
